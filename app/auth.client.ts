...
export async function submitRequest(payload: RequestPayload) {
    try {
        ...
        if (response.status === 201) {
            // Si la petició és exitosa (codi 201), retorna la resposta del servidor
            return response.data;  // Pot ser un missatge de confirmació o informació addicional
        } else {
            // Si la resposta no és correcta, llença un error
            throw new Error('Hi ha hagut un error en enviar la petició.');
        }
    } catch (error) {
